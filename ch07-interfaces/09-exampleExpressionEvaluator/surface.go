...
func main() {
	fmt.Println("Surface web-server using the eval expression to plot svg graphs")
	// evel package copied verbatim from book -> TODO: add comments there
	http.HandleFunc("/plot", plot)
	log.Fatal(http.ListenAndServe("localhost:8000", nil))
	// a) http://localhost:8000/plot?expr=sin(-x)*pow(1.5,-r)
	// b) http://localhost:8000/plot?expr=pow(2,sin(y))*pow(2,sin(x))/12
	// c) http://localhost:8000/plot?expr=sin(x*y/10)/10
}
...
func parseAndCheck(s string) (eval.Expr, error) {
	...
	// we have to check the out vars variable here!!!
	// not in the eval package....
	for v := range vars {
		if v != "x" && v != "y" && v != "r" {
